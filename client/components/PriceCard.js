...
function PriceCard({ props }) {
    return (
        <button className={`flex flex-col ${props.popular ? "h-[40%] md:h-5/6" : "h-[40%] md:h-4/6"} w-full md:w-[25%] mb-8 md:mb-0 shadow-lg shadow-slate-600 bg-slate-50 rounded-xl mx-3 p-5 transition-transform duration-300 hover:scale-105`}>
            <div className="h-full flex flex-col justify-center items-start">
                <h1 className="text-base md:text-lg text-black font-semibold uppercase">{props.title}</h1>
                {
                    props.price === "free" ? 
